...
from models.calming_evaluator import extract_features, predict_report, save_sidecar_outputs
...
# -------------------- Evaluator Sidecar Endpoints (Non-invasive) --------------------

@app.post("/api/model/evaluator/predict/{job_id}")
async def run_evaluator(job_id: str):
    """Run the ML evaluator sidecar for a completed job. Does not modify existing outputs."""
    if job_id not in jobs:
        raise HTTPException(status_code=404, detail="Job not found")
    ...
    # Locate outputs
    wave_path = job["output_files"].get("preprocessed_eeg")
    music_path = job["output_files"].get("music_parameters")
    midi_path = job["output_files"].get("midi_file")

    if not wave_path or not Path(wave_path).exists():
        raise HTTPException(status_code=404, detail="wave_analysis.json not found for this job")
    if not music_path or not Path(music_path).exists():
        raise HTTPException(status_code=404, detail="music_parameters.json not found for this job")

    # Compute features and report (read-only on originals)
    try:
        features = extract_features(Path(wave_path), Path(music_path), Path(midi_path) if midi_path else None)
        report = predict_report(features)
        # Save sidecar files
        output_paths = config.get('paths', 'output')
        sidecar_dir = Path(output_paths['json'])
        features_fp, report_fp = save_sidecar_outputs(features, report, sidecar_dir)

        # Track in job outputs (metadata only)
        job["output_files"]["features"] = str(features_fp)
        job["output_files"]["therapy_report"] = str(report_fp)

        return {
            "job_id": job_id,
            "features_path": str(features_fp),
            "report_path": str(report_fp),
            "message": "Evaluator completed. Sidecar files created."
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Evaluator failed: {str(e)}")


@app.get("/api/analysis/{job_id}")
async def get_analysis(job_id: str):
    """Fetch the evaluator sidecar report and features for a job if available."""
    if job_id not in jobs:
        raise HTTPException(status_code=404, detail="Job not found")

    job = jobs[job_id]
    features_fp = job["output_files"].get("features")
    report_fp = job["output_files"].get("therapy_report")

    # Fallback to default paths if not tracked
    output_paths = config.get('paths', 'output')
    default_features = Path(output_paths['json']) / 'features.json'
    default_report = Path(output_paths['json']) / 'therapy_report.json'

    features_data = None
    report_data = None

    if features_fp and Path(features_fp).exists():
        features_data = json.loads(Path(features_fp).read_text())
    elif default_features.exists():
        features_data = json.loads(default_features.read_text())

    if report_fp and Path(report_fp).exists():
        report_data = json.loads(Path(report_fp).read_text())
    elif default_report.exists():
        report_data = json.loads(default_report.read_text())

    if report_data is None and features_data is None:
        raise HTTPException(status_code=404, detail="No analysis found for this job. Run the evaluator first.")
